...
| --- | --- | --- | --- | --- | --- |
| R30.2 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 済 | 未実施（注14） |
| R30.2 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 済（注1） | 未実施（注14） |
| R30.2 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施（注14） |
...
- **注14**: R30.2は両連携構成でShader 6本、UdonSharp、ビルド時バリアントを
  ...
  **実機での目視確認は行っていません。**
  ...
  ありません。ただし**この変更が効くのは Udon Global 共有の経路で、描画検査は
  その経路を通っていません**（描画検査は `_MB_UDON_GLOBALS` を使わずMaterialの
  プロパティだけで描きます）。コンパイルは全Keyword ONで確認していますが、
  **誤ったサンプラーを引いてもコンパイルは通ります。**
...
**未実施**: R30.2で変えたサンプラー名の作り方の、Udon Global 共有時のVRChat PC実機目視
- 追跡ID: R30.2-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 描画検査は Udon を使わず、`_MB_UDON_GLOBALS` なしでMaterialのプロパティだけで描いています。R30.2で変えたのはサンプラー名の作り方で、**効くのはまさにその共有経路**のため、描画検査50シナリオでは通りません。コンパイルは全KeywordをONにしたMaterialで通っていますが、**誤ったサンプラーを引いてもコンパイルは通ります。**
- 期限: 2026-09-08
- 担当: 利用者とリリース担当
- 次アクション: Udon Global 共有をONにした状態（既定）で、光点形状テクスチャと光点形状アトラスがR30.1と同じに見えることをVRChat PCクライアントで確認し、この項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-01
- 延長回数: 0
