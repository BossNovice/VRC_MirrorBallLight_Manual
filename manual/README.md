...
**未実施**: 実機目視確認（R28.2 / 連携パッケージ未導入構成）
- 追跡ID: R28.2-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 連携パッケージを導入した2構成（VRCLV 2.1.3 + LTCGI 1.7.2、
  VRCLV 3.0.0-dev.15 + LTCGI 1.7.3）は実機で目視確認し、問題はありませんでした。
  連携パッケージ未導入の構成は、そのワールドが手元にないため確認していません。
- 期限: 2026-09-30
- 担当: @BossNovice
- 次アクション: 通常版Shaderだけを使うワールドが用意できた時点で
  [実機スモーク確認](実機スモーク確認.md) を実施する。用意できない場合は、
  確認しないと決めて表から行を外す
- 更新日: 2026-08-23
- 延長回数: 0
