...
- **注5**: R29.3.2で**新たに実機を見たわけではありません。** この版で変わるのは配布ZIPに
  同梱する `README_FIRST.txt` の表記だけで、**Assets の中身は R29.3.1 と1バイトも
  違いません**（`git diff v29.3.1 HEAD -- Assets/` が空であることを確認済み）。
  見た目を作るものが同一なので、R29.3.1 の確認をそのまま引き継いでいます。
  R28.4の注3と同じ扱いです。
