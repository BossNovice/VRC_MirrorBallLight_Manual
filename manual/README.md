...
| R30 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施（注12） |
...
- **注11**: R30は両連携構成でShader 6本、UdonSharp、ビルド時バリアントを検証し、
  ...
  2026-08-31に、**両連携構成のVRChat PCクライアントで合成方式4つ（従来の完全加算／
  VRCLightVolumesのみ／LTCGIのみ／推奨ハイブリッド）を切り替えて比較し、問題なしとの
  報告を受けています。**
  **確認したのはここまでです。** スペキュラ方式（主方向のみ／RGB方向別）、AudioLink動作中の
  明るさ、Additive Light Volume専用受光は目視していません。残りは `R30-UNEXEC-01` です。
  実機目視は代表的なシーンの確認であり、全機能を網羅しません。

- **注12**: 連携パッケージ未導入の構成では、**R30で追加した合成方式・反射方式・
  Additive受光を観察できません。** 連携版Shader3本を使えないためです（注2）。
  この行の実機目視は、通常版Shaderについての確認を指しますが、R30では実施していません。

**未実施**: R30のスペキュラ方式・AudioLink動作中の明るさ・Additive専用受光のVRChat PC実機目視
- 追跡ID: R30-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 自動検証ではUnityスタンドアロン描画まで確認できますが、VRChatクライアント上のAvatar Shaderや実ワールドのVolume／LTCGI構成を再現できません。Additive専用受光は、ワールドにAdditive Light Volumeが無いと確認できません。
- 期限: 2026-09-06
- 担当: 利用者とリリース担当
- 次アクション: [実機確認: R30の照明連携](実機確認_R30照明連携.md) の Case 5〜7 を実施し、同じ手順書の記録欄と、この項目およびバージョン対応表へ結果を書く
- 更新日: 2026-08-31
- 延長回数: 0
- 経過: 2026-08-31に Case 1〜4（合成方式4つの比較）を両連携構成で実施し、問題なしとの報告を受けました。残りは Case 5〜7 です。
