...
| MirrorBallLight | Unity | VRCLightVolumes | LTCGI | 状態 |
| --- | --- | --- | --- | --- |
| R27.3 | 2022.3.22f1 | 2.1.3 | 1.7.2 | バッチモードでコンパイル確認済み |
| R27.3 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | バッチモードでコンパイル確認済み |
| R27.3 | 2022.3.22f1 | 未導入 | 未導入 | 通常版Shaderのみ使用可（連携版はコンパイル不可） |

連携版Shaderを使う場合は、VRCLV 2.xにはLTCGI 1.7.2、VRCLV 3.0.0-dev.14以降にはLTCGI 1.7.3以降を組で揃えてください。
VRCLV 3.0.0-dev.12／dev.13はLTCGI非互換です。

上表はUnityバッチモードでのコンパイル確認結果です。VRChat実機での見え方は各自の環境で確認してください。

### 版ごとの移行時の注意

| 移行元 | 注意点 |
| --- | --- |
| R27.2 → R27.3 | ライブプリセットへ本体シード設定を保存できるようになり、演出プリセットの生成ボタンが増えます。既存プリセットの動作は変わりません |
| R27〜R27.1 → R27.3 | 上記に加えて、暗所で本体が黒く潰れる経路の修正が入ります。設定変更は不要で、正常時の見た目は変わりません |
| R26系 → R27.3 | 本体ファセットのシード自動フェードが追加されます。初期値OFFのため既存の見た目は変わりません |
| R25系 → R27.3 | 光点形状Atlasのシード自動フェードが追加されます。初期値OFFのため既存の見た目は変わりません |
| R24以前 → R27.3 | 壁・ガラスの表面Emissionが追加されます。初期値OFFのため既存の見た目は変わりません |
