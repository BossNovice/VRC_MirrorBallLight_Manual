...
- **注15**: R30.3で変えたのは**Unity Editor上で動く診断ウィンドウだけ**です。
  ...
  通っています。
  **Editorで実際にボタンを押す確認は未実施です**（下の R30.3-UNEXEC-01）。
...
**未実施**: 診断ウィンドウの「安全に自動修正」ボタンを、Unity Editorで実際に押す確認
- 追跡ID: R30.3-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 自動検査（`DiagnosticsFixTest`、11ケース43アサーション）は `AuditOpenScenes` を直接呼びます。**ボタンからそこへ至る経路と、`AssetDatabase.SaveAssets` によるディスクへの保存は通っていません。** 検査が使うMaterialはアセットではないため、保存が走らないためです。バッチモードのUnityではウィンドウ操作を再現できません。
- 期限: 2026-09-09
- 担当: 利用者とリリース担当
- 次アクション: 古いMaterialを含むプロジェクトで診断ウィンドウを開き、「安全に自動修正」を押して、警告が消えること・Unityを再起動しても直った状態が残っていること（保存されていること）を確認し、この項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-02
- 延長回数: 0
