...
| R28.2 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施 |
...
**未実施**: 実機目視確認（R28.2 / VRCLV 2.x構成・連携パッケージ未導入構成）
- 追跡ID: R28.2-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: VRCLightVolumes 3.0.0-dev.15 + LTCGI 1.7.3 の構成では実機で目視確認し、問題は
  ありませんでした。残る2構成（VRCLV 2.1.3 + LTCGI 1.7.2、連携パッケージ未導入）は、
  その構成のワールドが手元にないため確認していません。
- 期限: 2026-09-30
- 担当: @BossNovice
- 次アクション: 該当構成のワールドが用意できた時点で [実機スモーク確認](実機スモーク確認.md)
  を実施する。用意できない場合は、確認しないと決めて表から行を外す
- 更新日: 2026-08-23
- 延長回数: 0
