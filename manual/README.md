...
| --- | --- | --- | --- | --- | --- |
| R30.18 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 済 | 対象外（注30） |
| R30.18 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 済（注1） | 対象外（注30） |
| R30.18 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 対象外（注30） |
...
- **注30**: R30.18で直したのは2件です（Issue #127 / #128）。**どちらも描画を変えません。**
  ...
  **実機目視は対象外です。** 描画を変えていないためです。ただし、**受信側のずれは
  「範囲外の番号を受け取ったとき」にしか出ません**（プリセットの数を実行中に変えた、
  版が違う、など）。**その状況を実機で作っての確認はしていません。** ClientSimで
  `OnDeserialization` を走らせ、範囲外の値が書き戻らないことまでを見ています。
...
- **注29**: R30.17で直したのは、**きらめき・ランダム点滅・色相送り・Cookie回転の
  ...
  **実機目視は対象外です。** 描画も既定の動作も変えていないためです。ただし、
  **直したこと自体を実機で確かめられるのは16時間以上たってからです。** 検査で
