...
| --- | --- | --- | --- | --- | --- |
| R29.4.1 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 済 | 未実施（注7） |
| R29.4.1 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 済（注1） | 未実施（注7） |
| R29.4.1 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施（注7） |
...
この設定を有効にする場合は、[実機スモーク確認](実機スモーク確認.md) を行ってから公開してください。
...
**未実施**: R29.4.1 の実機目視（PC）
- 追跡ID: R29.4.1-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: VRChat（PC）の実機環境が用意できていません。R29.4.1 の変更は
  `電源の範囲` を「ローカル」にしたときだけ通る経路で、**初期値の「グローバル」では
  到達しません。** その経路については、Udonを実際に走らせる検査（ClientSim）で
  同期変数へ書き込まないことと、ローカル側の基準が動くことを確認しています。
  直す前のコードでは同じ検査が落ちることも確かめました。**ただしこれは実機の
  見え方を確認したものではありません。**
- 期限: 2026-09-30
- 担当: リポジトリ所有者
- 次アクション: `電源の範囲` を「ローカル」にした構成で、電源のON/OFFと範囲の切替を
  実機で行い、回転角が飛ばないことを目視する
- 更新日: 2026-08-28
- 延長回数: 0

- **注7**: R29.4.1 の実機目視は未実施です。上の未実施ブロックに理由と期限を記載しています。
  R29.4 までの実機目視の結果は、そのまま有効です（R29.4.1 で変わったのは
  `電源の範囲`＝ローカルのときの経路だけで、初期値の構成では挙動が変わりません）。
...
**既定はOFFです。** 有効にしない限りR28.3から動作は変わりません。
...
R29から、光点がまったく映らない面では光点の計算そのものを行わなくなりました。設定は不要で、見た目も変わりません。
...
### 半透明版も少し軽くなりました（R29.1）

R29.1から、半透明版のShaderでも色の計算を「光点が映らない画素では行わない」ようにしました。設定は不要で、見た目も変わりません。

効く条件はR29と同じです（`距離フェードを使用` がONで `フェード終了距離` より遠い面、または電源がOFFのとき）。
...
**R29から、距離フェードは軽量化のスイッチになりました。** 終了距離より遠い面では、光点の形・色・点滅の計算そのものを行いません。
