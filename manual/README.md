...
| --- | --- | --- | --- | --- | --- |
| R30.7 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 済 | 未実施（注19） |
| R30.7 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 済（注1） | 未実施（注19） |
| R30.7 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施（注19） |
| R30.6 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 済 | 未実施（注18） |
| R30.6 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 済（注1） | 未実施（注18） |
| R30.6 | 2022.3.22f1 | 未導入 | 未導入 | 済（注2） | 未実施（注18） |
...
- **注18**: R30.6で直したのは**プリセット切替の所有権の取り方**です。同期変数を
  ...
  **未実施**とし、追跡項目（R30.6-UNEXEC-01）を立てています。
...
**未実施**: R30.7で直した「長時間起動での更新停止」が実機で解消しているかの確認
- 追跡ID: R30.7-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: `Time.time` が大きい状態（VRChatを数日起動したまま）でしか出ません。
  **バッチモードのUnityでは、その時間経過を再現できません。** float32での計算では
  確認済みですが、実機で更新頻度が戻ることは確認していません。
- 期限: 2026-09-11
- 担当: 利用者とリリース担当
- 次アクション: VRChatを数日起動したままの状態でワールドへ入り、(1) 回転が
  なめらかに続くこと、(2) プリセットのクロスフェードが飛ばないこと、
  (3) AudioLinkが反応することを確認してこの項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-04
- 延長回数: 0

**未実施**: R30.6で直したプリセット切替が、複数人のワールドで正しく伝わるかの確認
- 追跡ID: R30.6-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 所有権の受け渡しは**2人以上が同じインスタンスにいないと起きません。**
  ClientSimのリモートプレイヤーで所有権の移動までは検査していますが、**実際の
  ネットワーク越しの伝わり方は確認していません。**
- 期限: 2026-09-11
- 担当: 利用者とリリース担当
- 次アクション: 2人以上でワールドへ入り、(1) ワールドの所有者でない人がプリセットを
  切り替えたとき全員に反映されること、(2) 切り替えた直後に回転が飛んだり止まったり
  しないこと、を確認してこの項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-04
- 延長回数: 0

**未実施**: R30.5で直した「回転が止まる」がVRChatで実際に直っているかの確認
- 追跡ID: R30.5-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 直した現象は**VRChatを長時間起動している人ほど早く止まる**というもので、
  Unityのバッチモードでは `_Time.y` が0のまま止まるため再現できません。float32での
  計算と故障注入までは確認済みですが、**実機で回り続けることは確認していません。**
- 期限: 2026-09-11
- 担当: 利用者とリリース担当
- 次アクション: R30.5へ更新したワールドで、(1) ミラーボールと光点が回り続けること、
  (2) プリセットを切り替えても止まらないこと、(3) VRChatを長時間起動したままの状態で
  入っても回ること、を確認してこの項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-04
- 延長回数: 0

**未実施**: 自動生成したUIボタンが押せるかの確認と、新しい診断の表示
- 追跡ID: R30.5-UNEXEC-02
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: 診断そのものは判定テスト12件で確認していますが、**Sceneから値を読む側
  （MirrorBallValidationReport）は自動検査を持ちません。** 実際のCanvasに対して
  正しく判定するかは、Unity上で診断ウィンドウを開かないと確かめられません。
- 期限: 2026-09-11
- 担当: 利用者とリリース担当
- 次アクション: 診断ウィンドウを実行し、押せないボタンに対して MBL-W021〜W030 が
  出ること、押せるボタンに対して出ないことを確認する。VRChatでボタンが効くことも確認する
- 更新日: 2026-09-04
- 延長回数: 0
