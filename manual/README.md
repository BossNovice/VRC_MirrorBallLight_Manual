...
| --- | --- | --- | --- | --- | --- |
| R30.15 | 2022.3.22f1 | 2.1.3 | 1.7.2 | 一部のみ（注27） | 対象外（注27） |
| R30.15 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | 一部のみ（注27） | 対象外（注27） |
| R30.15 | 2022.3.22f1 | 未導入 | 未導入 | 一部のみ（注27） | 対象外（注27） |
...
- **注27**: R30.15は、**役目を終えた診断ログを消した**だけです。R30.11で入れたもので、
  実機ログから原因を特定し、直ったことも確認できたため不要になりました。
  **Inspectorの確定処理の位置（R30.13の修正）はそのまま残しています。** 描画にも
  既定の動作にも影響しないため、実機目視は対象外です。
...
**未実施**: R30.9・R30.10で省略した検査（描画差分・ClientSim・Import時間・配布物）
- 追跡ID: R30.9-UNEXEC-02
- 対象: R30.9 と R30.10 の両方（同じ状態です）
- 理由種別: TIMEBOX
- 理由詳細: 利用者からの「リリースを急ぎたいので全パターンテストは省略」という
  判断により、**意図して省きました。** 環境の制約ではありません
- 期限: 2026-09-11
- 担当: リリース担当
- 次アクション: 次のリリースでリリース判定46段階を通し、R30.9で入れた変更が
  描画差分・ClientSim検査を通ることを確認する
- 更新日: 2026-09-04
- 延長回数: 0

**未実施**: R30.9で、VRChatで回転するようになったことの確認（R30.5〜R30.8の修正もまとめて見ます）
