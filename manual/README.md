...
- **注16**: R30.4で変えたのは**Shaderのチェックボックスの出し方（`[Toggle]` →
  ...
  **新しく足した「LTCGIだけOFF」ボタンをEditorで実際に押す確認は未実施です**
  （下の R30.4-UNEXEC-01）。
...
**未実施**: 「LTCGIだけOFF（VRCLightVolumesは残す）」ボタンを、Unity Editorで実際に押す確認
- 追跡ID: R30.4-UNEXEC-01
- 理由種別: ENV_UNAVAILABLE
- 理由詳細: このボタンは**利用者のMaterialを書き換えて保存します**。同種の経路（診断の自動修正）には検査を足しましたが、**このボタンには自動検査がありません。** バッチモードのUnityではウィンドウ操作を再現できません。
- 期限: 2026-09-09
- 担当: 利用者とリリース担当
- 次アクション: Controllerの「9. VRCLightVolumes・LTCGI連携」で「LTCGIだけOFF」を押し、対象Materialの `LTCGIを使用` だけがOFFになり **`VRCLightVolumesを使用` は残っている**こと、Unityを再起動しても残ることを確認し、この項目とバージョン対応表へ結果を書く
- 更新日: 2026-09-02
- 延長回数: 0
