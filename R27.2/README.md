# VRC MirrorBallLight R27.2 GitHub版マニュアル
...
| --- | --- | --- | --- | --- |
| R27.2 | 2022.3.22f1 | 2.1.3 | 1.7.2 | バッチモードでコンパイル確認済み |
| R27.2 | 2022.3.22f1 | 3.0.0-dev.15 | 1.7.3 | バッチモードでコンパイル確認済み |
| R27.2 | 2022.3.22f1 | 未導入 | 未導入 | 通常版Shaderのみ使用可（連携版はコンパイル不可） |
...
| 移行元 | 注意点 |
| --- | --- |
| R27〜R27.1 → R27.2 | 設定変更は不要です。暗所で本体が黒く潰れる経路の修正のみで、正常時の見た目は変わりません |
| R26系 → R27.2 | 本体ファセットのシード自動フェードが追加されます。初期値OFFのため既存の見た目は変わりません |
| R25系 → R27.2 | 光点形状Atlasのシード自動フェードが追加されます。初期値OFFのため既存の見た目は変わりません |
| R24以前 → R27.2 | 壁・ガラスの表面Emissionが追加されます。初期値OFFのため既存の見た目は変わりません |
